...
struct compactor {
	...
	linkedList* getCompactor(int compactorNumber) {
        ...
        linkedList* COMPACT(int compactorNumber) {
                cNode* _head = head;
                while (compactorNumber > 1) {
                        compactorNumber--;
                        _head = _head->next;
		...
		if (random == 0) {
                        while (_head->val->len >= 2) {
                                _head->val->pop();
                                _list->insert(_head->val->yieldPop());
                        }
                }
                else {
                        while (_head->val->len >= 2) {
...
void AggregateFunctions::MedianUpdate(FunctionContext* ctx, const DoubleVal& src, StringVal* dst) {
...
void AggregateFunctions::MedianMerge(FunctionContext* ctx, const StringVal& src, StringVal* dst) {
...
DoubleVal AggregateFunctions::MedianFinalize(FunctionContext* ctx, const StringVal& src) {
  compactor state = *reinterpret_cast<compactor*>(src.ptr);
  ctx->Free(src.ptr);
  if (total == 0) return DoubleVal::null();
...
void AggregateFunctions::CorrUpdate(FunctionContext* ctx, const DoubleVal& src1, const DoubleVal& src2, 
		StringVal* dst) {
...
void AggregateFunctions::CorrMerge(FunctionContext* ctx, const StringVal& src, StringVal* dst) {
...
const StringVal AggregateFunctions::CorrSerialize(FunctionContext* ctx, const StringVal& src) {
