...
// In this sample, we are trying to find out the distance between 2 points
// We are assuming that the points are simple integers
DoubleVal distanceUDF(FunctionContext* context, const StringVal& arg1, const StringVal& arg2)
{
        if (arg1.is_null || arg2.is_null) return DoubleVal::null();

	std::string p1((const char *)arg1.ptr,arg1.len);
	std::string p2((const char *)arg2.ptr,arg2.len);

        int pos1 = p1.find(",");
        int pos2 = p2.find(",");
        if (pos1 == std::string::npos || pos2 == std::string::npos) return DoubleVal::null();

        std::string _x1 = p1.substr(0,pos1);
        std::string _y1 = p1.substr(pos1+1,arg1.len);

        std::string _x2 = p2.substr(0,pos2);
        std::string _y2 = p2.substr(pos2+1,arg2.len);
        ...
        double result = sqrt(double(pow((x2 - x1),2) + pow((y2 - y1),2)));

        return DoubleVal(result);
}


// In this sample we are declaring a UDF that subtracts two ints and returns an int.
IntVal SubUdf(FunctionContext* context, const IntVal& arg1, const IntVal& arg2) {
  if (arg1.is_null || arg2.is_null) return IntVal::null();
  if(arg1.val >= arg2.val) return IntVal(arg1.val - arg2.val);
  if(arg1.val < arg2.val) return IntVal(arg2.val - arg1.val);
}
...
int stoInt (std::string & s) {
	...
	for (int i = 0; i < s.length(); i++) {
		int temp = (int)s[i] - 48;
		if (temp >= 0 && temp <= 9) sum += temp * pow(10,index--);
		else index--;
	}
	return sum;
}
